...
export const LoadingIndicator = ({ type = "line-simple", size = "sm", label }: LoadingIndicatorProps) => {
    const renderSpinner = () => {
        ...
        return (
            <svg className={cx("animate-spin", styles[size].spinner)} viewBox="0 0 32 32" fill="none">
                <circle className="text-gray-200" cx="16" cy="16" r="14" stroke="currentColor" strokeWidth="4" />
                <circle
    ...
    return (
        <div className={cx("flex flex-col h-full items-center justify-center", styles[size].root)}>
            {renderSpinner()}
            {label && <span className={cx("text-second-text", styles[size].label)}>{label}</span>}
        </div>
    );
};
